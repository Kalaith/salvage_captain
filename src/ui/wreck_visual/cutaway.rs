...
pub(super) fn draw_shell(wreck: Rect, accent: Color) {
    ...
    // A broad, broken silhouette rather than a boxed diagram.
    let outline = [
        vec2(x + 60.0, y + 30.0),
        vec2(x + 132.0, y),
        vec2(right - 92.0, y),
    ...
    let interior = Rect::new(x + 100.0, y + 38.0, wreck.w - 160.0, wreck.h - 80.0);
    draw_rectangle(
        interior.x,
        interior.y,
        interior.w,
    ...
    for index in 0..12 {
        let rivet_x = x + 152.0 + index as f32 * 55.0;
        draw_circle(rivet_x, y + 14.0, 2.0, theme::structure_dark());
        draw_circle(rivet_x, bottom - 20.0, 2.0, theme::structure_light());
    }
}
